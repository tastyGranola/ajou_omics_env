...
> 이 저장소가 쓰는 버전은 **decoupler 2.2** 입니다.
> `decoupler 1.x` 와 함수 이름이 **전부 다릅니다.** 아래 표를 먼저 보세요.

## ⚠ 1.x 코드가 나오면 잘못된 것입니다

LLM 이 학습한 코드 대부분은 1.x 입니다. 아래 왼쪽이 나오면 오른쪽으로 고칩니다.
...
print(dc.__version__)      # 2.2.x 여야 합니다
...
챕터가 반복하는 뼈대입니다. 코드를 짜기 전에 이 셋이 무엇인지 먼저 적으세요.

| | 무엇 | 이 실습에서 |
|---|---|---|
| **readout** | 분자 수준 측정값 — 발현 행렬 또는 유전자별 통계량 | 세포×유전자 log-normalized, 또는 pseudobulk DE 의 Wald stat 한 줄 |
| **prior knowledge** | 유전자 집합(gene set) 또는 발자국(footprint) | Hallmark · Reactome · PROGENy · CollecTRI |
...
| **조건 대비** | DataFrame 한 줄 (대비 × 유전자, DE 통계량) | `gsea` · `ulm` · `ora` | stim 이 ctrl 대비 **무엇을 켰나** |

**둘은 다른 질문에 답합니다.** 세포 수준 점수를 조건별로 평균 내는 것은 조건 대비가 아닙니다
(세포를 독립 표본으로 취급하게 되어 pseudoreplication 이 그대로 남습니다).
조건을 비교하려면 **pseudobulk → DE → 통계량을 readout 으로** 넘기세요.

---

## 3. 입력 형태에 따라 반환값이 달라진다

`dc.mt.*` 는 모두 같은 규칙을 씁니다.
...
`aucell` 은 p 값이 없습니다 — `score_aucell` 만 생기고 `padj_aucell` 은 없습니다.

`obsm` 에 든 점수를 scanpy 로 그리려면 AnnData 로 꺼냅니다.
...
> **`tmin` 을 그냥 두지 마세요.** 챕터의 권고는 **10~15개 미만 집합을 버리는 것**입니다.
> 작은 집합은 분산이 커서 검정 통계량이 불안정해집니다.
> gene set 에는 `tmin=15`, footprint 에는 기본값(5) 그대로 두세요.
> **CollecTRI 는 1,185개 TF 중 716개의 target 이 15개 미만**(중앙값 9)이라
> `tmin=15` 를 걸면 TF 의 60%가 사라집니다. footprint 는 애초에 target 수가 적게 설계돼 있습니다.
> (PROGENy 는 `top=500` 이라 202~500개로 영향이 없습니다.)
>
> 상한(예: 500개 초과 제외)은 `tmin` 이 해주지 않습니다. `net` 을 직접 걸러야 합니다.
...
`seed` 를 고정하지 않으면 `gsea` 결과가 실행할 때마다 조금씩 달라집니다. **반드시 고정하세요.**
...
`dc.op.*` 는 **omnipathdb.org 에 네트워크 요청을 보냅니다.** 오프라인이거나 느리면
미리 받아 둔 캐시를 쓰세요.

```bash
python3 parallel_lab/fetch_genesets.py       # data/genesets/ 에 내려받아 캐시
python3 parallel_lab/verify.py               # 무엇이 캐시돼 있는지 확인
```
...
> **`dc.op.hallmark()` 은 `HALLMARK_` 접두어를 떼고 돌려줍니다.**
> source 이름은 `INTERFERON_ALPHA_RESPONSE` 이지 `HALLMARK_INTERFERON_ALPHA_RESPONSE` 가
> 아닙니다. GMT 파일에서 읽은 이름과 섞어 쓰면 조용히 빈 결과가 나옵니다.
> 무엇이 들었는지는 항상 먼저 확인하세요.
>
> ```python
...
> ```
>
> Hallmark 50개 집합의 크기는 **32~200개**(중앙값 199)입니다 — `tmin=15` 로는 하나도 안 잘립니다.
> 반면 Reactome 은 2,105개 중 **710개**가 15개 미만이라 3분의 1이 잘립니다.
> 잘린 개수를 항상 확인하세요.

Reactome 를 omnipath 에서 받으려면 `resource("MSigDB")` 를 걸러야 합니다.
`dc.op.resource` 는 `source`/`target` 이 아니라 **wide 형식**(`genesymbol` · `collection` · `geneset`)을
돌려주므로 직접 이름을 바꿔야 합니다.
...
**gene set 은 "이 경로의 구성원"** 이고, **footprint 는 "이 경로가 켜지면 변하는 유전자"** 입니다.
전사체 데이터에는 footprint 가 더 직접적으로 대응합니다 — 경로 단백질의 mRNA 가
그 경로의 활성을 반영한다는 보장이 없기 때문입니다.

---

## 7. pseudobulk → DE → 조건 대비 농축 (이 실습의 본선)

```python
# 7-1. 세포 타입 안에서 (도너 × 조건) 으로 합친다. 입력은 raw count 여야 한다.
pdata = dc.pp.pseudobulk(
    adata[adata.obs["cell_type"] == "CD14+ Monocytes"],
    sample_col="donor",          # 생물학적 반복 = 도너
    groups_col="stim",           # 조건
...
#      아래는 pydeseq2 0.5.x 기준입니다 (0.4 는 design 대신 design_factors 를 씁니다)
...
**7-1 의 `layer="counts"` 가 가장 흔한 실수 지점입니다.** `pseudobulk` 는 count 를 더하는
것이 기본 동작이고, pydeseq2 도 count 를 요구합니다. log-normalized 값을 더하면 둘 다 틀립니다.
...
> AUCell·z-score 처럼 **순위/분포에 기대는 방법은 정규화 방식에 민감합니다.**
> 정규화를 바꿔서 결과가 크게 흔들리면 그 자체가 보고할 관찰입니다.
...
`dc.pl.leading_edge` 는 **그림과 함께 leading-edge 유전자 배열을 돌려줍니다.**
"이 경로가 나온 이유가 무엇인가" 를 유전자 이름으로 답할 수 있는 유일한 지점이니
상위 경로 두세 개에 대해서는 반드시 돌려서 표로 남기세요.
...
| 상위 경로가 서로 거의 같은 유전자 | gene set 중복 (챕터의 명시적 경고) | Hallmark 로 바꾸거나, 상위 집합 간 Jaccard 를 표로 남긴다 |
...
5번이 프롬프트 7.4 의 근거입니다 — **한 실험 안에서** gene set 을 둘 이상, 통계 방법을
둘 이상 써서 상위 결과의 일치도를 재고, 무엇을 바꿨을 때 더 갈렸는지 적게 합니다.
실험을 늘리지 않고도 확인할 수 있는 주장입니다.
